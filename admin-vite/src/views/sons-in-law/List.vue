  ...
  <div class="sons-in-law-list">
    <el-card>
      ...
      <el-form :inline="true" :model="searchForm" class="search-form">
        <el-form-item label="妻子ID">
          <el-input v-model="searchForm.wifeId" placeholder="妻子ID" clearable />
        </el-form-item>
      ...
      <el-table :data="tableData" border style="width: 100%" v-loading="loading" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="sonId" label="女婿ID" width="120">
          <template #default="scope">
            <el-tag type="info">{{ scope.row.sonId || '-' }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="wifeName" label="妻子" width="120" />
        <el-table-column prop="hometown" label="籍贯/入赘地" min-width="150" show-overflow-tooltip />
...
async function init() {
  try {
    ...
    cloudInitialized.value = success
    if (success) {
      loadData()
    }
  } catch (error) {
...
const loadData = async () => {
  ...
  try {
    const res = await sonsInLawApi.getList({
      wifeId: searchForm.wifeId || null,
...
const goToAdd = () => {
  router.push('/sons-in-law/form')
}

const handleEdit = (row) => {
  router.push('/sons-in-law/form?id=' + row._id)
}
...
const handleDelete = async (row) => {
  try {
    ...
    const res = await sonsInLawApi.delete(row._id)
...
const batchDelete = async () => {
  ...
  try {
    ...
    for (const row of selectedRows.value) {
      try {
        if (row._id) {
          const result = await sonsInLawApi.delete(row._id)
          if (result.success) {
            successCount++
          } else {
            failCount++
